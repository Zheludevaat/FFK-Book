...
class PipelineUI:
    ...
    def ask_user(self, prompt: str) -> str:
        ...
        if "Continue anyway" in prompt:
            return "y" if self.auto_continue_draft.get() else "n"
        if "Checkpoint issues" in prompt:
    ...
    def load_settings(self):
        ...
        if cfg_path.exists():
            ...
            ui = data.get("ui", {})
            self.auto_rewrite.set(ui.get("auto_rewrite", False))
            self.auto_continue_draft.set(ui.get("auto_continue_draft", False))
            self.auto_continue_checkpoint.set(ui.get("auto_continue_checkpoint", False))
    ...
    def save_settings(self):
        data = {
            ...
            "ui": {
                "auto_rewrite": self.auto_rewrite.get(),
                "auto_continue_draft": self.auto_continue_draft.get(),
                "auto_continue_checkpoint": self.auto_continue_checkpoint.get(),
            },
        }
